...
/*
     include_once('../mysql.php');//or include('../mysql.php')
     $current_date=date("Y-m-d");
     $sql="SELECT * FROM source WHERE url='{$_SESSION['source']}' AND visit_date='{$current_date}' AND cookie_value='{$_COOKIE['user']}'";
     $result=$conn->query($sql);
     if(mysqli_num_rows($result)>0)
    {
        $row=mysqli_fetch_assoc($result);
      ...
      $conn->close();
            ...
            $sql="INSERT INTO source(url,country,cookie_value,visit_date,click_count) VALUES('{$_SESSION['source']}','{$_SESSION['country']}','{$_COOKIE['user']}',CURDATE(),1)";
            $conn->query($sql);
          $conn->close();
     }
